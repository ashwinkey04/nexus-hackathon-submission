...
class _RAGPageState extends State<RAGPage> {
  ...
  Future<void> initializeModel() async {
    ...
    try {
      ...
      setState(() {
        isModelLoaded = true;
        outputText =
            'Model initialized successfully! Click "Initialize RAG" to set up the database.';
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> initializeRAG() async {
    ...
    try {
      ...
      setState(() {
        ...
        outputText =
            'RAG initialized successfully! Click "Add Docs" to populate the database.';
      });
      await getDBStats();
    } catch (e) {
      setState(() {
  ...
  Future<void> searchDocuments() async {
    ...
    try {
      final results = await rag.search(text: _queryController.text, limit: 2);
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Expanded(
                                child: Text(
                                  result.chunk.document.target?.fileName ??
                                      'Unknown Document',
                                  style: const TextStyle(
                                      fontWeight: FontWeight.bold,
                                      color: Colors.black),
